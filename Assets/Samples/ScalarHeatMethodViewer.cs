...
namespace VectorField.Demo {
    ...
    public class ScalarHeatMethodViewer : MonoBehaviour {
        ...
        void Start() {
            ...
            if(!c.hasStartedTexturing)
                Debug.LogError("Has not started texturing!");
            var g = c.geom;
            
            Debug.Log("number of geom faces:" + g.nFaces);
            ...
            {
                Debug.Log("dist I:" + dist[i]);
            }
            
            if (max != 0) {
                for(var i = 0; i < g.nVerts; i++) {
                    if (!double.IsNaN(dist[i])) {
                        cols[i] = colScheme.Evaluate((float)(dist[i] / max));
                    } else {
                        // Handle the case where dist[i] is NaN
                        // For example, you could set cols[i] to a default color
                        cols[i] = Color.black;
                    }
                }
            } else {
                // Handle the case where max is 0
                // For example, you could set all colors to a default color
                for(var i = 0; i < g.nVerts; i++) {
                    cols[i] = Color.black;
                }
            }
            ...
            if (tracers.Count > 0) {
                ...
                // Handle the case where tracers is empty
                // For example, you could log a warning message
                Debug.LogWarning("Tracers list is empty. GraphicsBuffers were not created.");
            }
